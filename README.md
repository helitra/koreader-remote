...
only a convenient address share, not authentication.
...
Updates are manual: the plugin never polls GitHub in the background. Before an
update replaces the plugin, it verifies the release checksum, archive layout,
and Lua syntax, then keeps the previous version as a backup until the new one
has started successfully. Only stable releases are offered.
